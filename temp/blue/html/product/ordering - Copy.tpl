...
<div class="cart_content">
    ...
    <table class="cart_table" width="100%" border="0" cellpadding="8" cellspacing="1">
      ...
      <!-- END: row_item --> 
      <!-- BEGIN: row_empty -->
      <tr><td class="col col_empty" colspan="7">{row.mess}</td></tr>
      <!-- END: row_empty --> 
      <tr>
        <td class="col col_total cart_total" align="right" colspan="7" data-min_cart_promotion="{data.min_cart_promotion}">
          <span class="col-title">{LANG.product.cart_total}:</span>
          <span class="col-content">{data.cart_total}</span>
        </td>
      </tr>
      <tr>
        <td class="col col_total" align="right" colspan="7">
          <span class="col-title">{LANG.product.promotional_code}:</span>
          <span class="col-content cart_promotion" value="{data.promotion_percent}">-<span class="number">{data.promotion_percent}</span>%</span>
        </td>
      </tr>
      <tr>
        <td class="col col_total" align="right" colspan="7">
          <span class="col-title">{LANG.product.gift_voucher}:</span>
          <span class="col-content cart_voucher" value="{data.voucher_amount}">-{data.voucher_amount_out}</span>
        </td>
      </tr>
      <tr>
        <td class="col col_total cart_payment" align="right" colspan="7">
  ...
  <div class="box_code">
  	<div class="box_code-title">{LANG.product.promotional_code}</div>
  	<div class="box_code-note">{LANG.product.promotional_code_note}</div>
  	<div class="box_code-content">
    	{data.err_promotion}
    	<form name="form_code" method="post" action="" >
        <label>{LANG.product.promotional_code_label}</label>
        <input name="promotional_code" type="text" value="" class="input_text" />
        <div class="clear"></div>
        <input type="submit" class="btn" value="{LANG.product.btn_use_code}"/>
        <div class="clear"></div>
      </form>
    </div>
  </div>
  
  <div class="box_code last">
  	<div class="box_code-title">{LANG.product.gift_voucher}</div>
  	<div class="box_code-note">{LANG.product.gift_voucher_note}</div>
  	<div class="box_code-content">
    	{data.err_voucher}
    	<form name="form_code" method="post" action="" >
        <label>{LANG.product.gift_voucher_label}</label>
        <input name="gift_voucher" type="text" value="" class="input_text" />
        <div class="clear"></div>
        <input type="submit" class="btn" value="{LANG.product.btn_use_code}"/>
        <div class="clear"></div>
      </form>
...
<div class="ordering_user">
    ...
    <div class="ordering_signin">{data.form_signin}</div>
    <div class="skip_login"><input type="button" class="btn" value="{LANG.product.btn_skip_login}" onclick="go_link('{data.link_address}/?skip_login=1');"/></div>
  </div>
  <div class="ordering_user_r">
  	<div class="ordering_user-title">{LANG.product.form_signup_title}</div>
    <div class="ordering_signup">
    	<div id="ordering_signup-info">
      	{data.signup_info}
        <input type="button" class="btn btn_signup_info" value="{LANG.global.btn_signup_info}" onclick="ttHUser.show_signup('form_signup','ordering_signup-info');"/>
      </div>
      {data.form_signup}
    </div>
  </div>
