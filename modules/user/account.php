	...
	**/
	...
	function do_main ()
	{
		...
		if (isset($ttH->post['do_submit'])) {
			...
			if(empty($err)){
				$col = array();
				$col["nickname"] = $ttH->post["nickname"];
				$col["phone"] = $ttH->post["phone"];
				$col["address"] = $ttH->post["address"];
				$col["province"] = $ttH->post["province"];
				$col["district"] = $ttH->post["district"];
				$col["ward"] = $ttH->post["ward"];
		...
		$data["list_location_province"] = $ttH->site_func->list_location_province ("province", 'vi', $data['province']," class=\"form-control select_location_province\" data-district='district' data-ward='ward' id='province'", array('title' => $ttH->lang['global']['select_title']));
		$data["list_location_district"] = $ttH->site_func->list_location_district ("district", $data['province'], $data['district']," class=\"form-control select_location_district\" data-ward='ward' id='district'", array('title' => $ttH->lang['global']['select_title']));
		$data["list_location_ward"] = $ttH->site_func->list_location_ward ("ward", $data['district'], $data['ward']," class=\"form-control\" id='ward'", array('title' => $ttH->lang['global']['select_title']));
		
		$ttH->temp_act->assign('data', $data);
		$ttH->temp_act->parse("account");
		return $ttH->temp_act->text("account");
	}
