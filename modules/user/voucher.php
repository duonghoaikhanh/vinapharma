...
class sMain
{
	...
	**/
	...
	function manage_row($row)
	{
		...
		$row['amount'] = $ttH->func->get_price_format($row['amount'], 0);
		$row['amount_use'] = $ttH->func->get_price_format($row['amount_use'], 0);
		$row['date_create'] = date('d/m/Y',$row['date_create']);
		$row['date_end'] = date('d/m/Y, H:i',$row['date_end']);
		
		$ttH->temp_act->assign('row', $row);
		
		$ttH->temp_act->parse("voucher.row_item");
		$output = $ttH->temp_act->text("voucher.row_item");
		$ttH->temp_act->reset("voucher.row_item");
	...
	function do_manage($is_show="")
	{
		...
		$num_total = 0;
		$res_num = $ttH->db->query("select voucher_id from voucher ".$where." ");
    ...
    if ($num = $ttH->db->num_rows($result))
		...
		else
		{
			...
			$ttH->temp_act->parse("voucher.row_empty");
		}
		...
		$ttH->temp_act->assign('data', $data);
		$ttH->temp_act->parse("voucher");
		return $ttH->temp_act->text("voucher");
	}
