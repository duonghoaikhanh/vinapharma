...
function html_list_group ($arr_in = array())
{
	...
	$link_action = (isset($arr_in['link_action'])) ? $arr_in['link_action'] : $ttH->site->get_link ('service');
	$temp = (isset($arr_in['temp'])) ? $arr_in['temp'] : 'list_item';
	$p = (isset($ttH->input["p"])) ? $ttH->input["p"] : 1;
	$pic_w = 164;//(isset($ttH->setting['service']["img_list_w"])) ? $ttH->setting['service']["img_list_w"] : 100;
	$pic_h = 164;//(isset($ttH->setting['service']["img_list_h"])) ? $ttH->setting['service']["img_list_h"] : 100;
	$num_row = 1;
	...
	$n = (isset($ttH->setting['service']["num_list"])) ? $ttH->setting['service']["num_list"] : 30;
	...
	if ($num = $ttH->db->num_rows($result))
	{
		...
		while ($row = $ttH->db->fetch_row($result)) 
		{
			...
			$row['pic_w'] = $pic_w;
			$row['pic_h'] = $pic_h;
			$row['link'] = $ttH->site->get_link ('service',$row['friendly_link']);
	...
	{
		$ttH->temp_act->assign('row', array("mess"=>$ttH->lang["service"]["no_have_item"]));
...
function box_menu () {
	...
	if(!isset($ttH->data["service"])){
		...
		$ttH->data["service"] = array();
		if($num = $ttH->db->num_rows($result)){
			while($row = $ttH->db->fetch_row($result)){
				$ttH->data["service"][$row["item_id"]] = $row;
			}
		}
	}
	...
	if(count($ttH->data["service"]) > 0){
		$data = array(
			'title' => $ttH->lang['service']['menu_title'],
			'content' => ''
		);
		
		$menu_sub = '';
		$i = 0;
		foreach($ttH->data["service"] as $row)
		{
			$i++;
			$row['link'] = $ttH->site->get_link ('service','',$row['friendly_link']);
