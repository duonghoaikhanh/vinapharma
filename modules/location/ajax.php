...
class sMain
{
	...
	**/
	...
	function do_load_ward_op ()
	{
		...
		if(!empty($ttH->post['district_code'])) {
			$where .= " and district_code='".$ttH->post['district_code']."'";
		}
		...
		$data = $ttH->load_data->data_table ('location_ward', 'code', 'code,title', "is_show=1 and lang='".$ttH->conf['lang_cur']."' ".$where."");
