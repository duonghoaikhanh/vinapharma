...
$ttH->conf['cur_act'] = (isset($ttH->conf['cur_act'])) ? $ttH->conf['cur_act'] : "project";
...
function load_setting ()
{
	...
	if(!isset($ttH->setting['project'])){
		$ttH->setting['project'] = array();
		$result = $ttH->db->query("select * from project_setting ");
		while($row = $ttH->db->fetch_row($result)){
			$ttH->setting['project_'.$row['lang']] = $row;
			if($ttH->conf['lang_cur'] == $row['lang']) {
				$ttH->setting['project'] = $row;
			}
		}
	}
	
	$ttH->data['project_group'] = (isset($ttH->data['project_group'])) ? $ttH->data['project_group'] : array();
	$query = "select p.group_id, title, friendly_link 
