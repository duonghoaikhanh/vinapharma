...
class sMain
{
	...
	**/
		...
		if(isset($ttH->conf['cur_group'])){
			...
			if($row = $ttH->db->fetch_row($result)){
				...
				$ttH->navigation = $ttH->site->html_arr_navigation (array(
					...
					array(
						'title' => $ttH->data['project_group'][$ttH->conf['cur_group']]['title'],
						'link' => $ttH->site->get_link ('project',$ttH->data['project_group'][$ttH->conf['cur_group']]['friendly_link'])
					)
				));
			...
			$ttH->site->get_seo (array(
				'meta_title' => (isset($ttH->setting['project']["project_meta_title"])) ? $ttH->setting['project']["project_meta_title"] : '',
				'meta_key' => (isset($ttH->setting['project']["project_meta_key"])) ? $ttH->setting['project']["project_meta_key"] : '',
				'meta_desc' => (isset($ttH->setting['project']["project_meta_desc"])) ? $ttH->setting['project']["project_meta_desc"] : ''
			));
	...
	function do_list ()
	{
		...
		$arr_in = array(
			'link_action' => $ttH->site->get_link ('project'),
			'where' => $where,
			'temp' => 'list_item',
		);
		
		$info = array();
		$info['type_show'] = 'grid_item';
		$info['num_show'] = 0;
	...
	function do_list_group ($info = array(), $info_lang = array())
	{
		...
		$arr_in = array(
			'link_action' => $ttH->site->get_link ('project',$info_lang['friendly_link']),
