...
<form action="{data.link_action}" method="post" enctype="multipart/form-data" name="myForm" id="myForm" role="form">
  <div class="row">
    ...
    <div class="col-md-6">
      <div class="form-group">
        <label>{row.title}</label>
        <textarea name="{row.key}" id="{row.key}" class="form-control" rows="3">{row.content}</textarea>
      </div>
      <p>{row.note}</p>
    </div>
    <!-- END: textarea --> 
    ...
    <div class="col-md-6">
      <div class="form-group">
        <label>{row.title}</label>
        <input name="{row.key}" id="{row.key}" type="text" size="50" maxlength="150" value="{row.content}" class="form-control">
      </div>
    ...
    <div class="box_search">
      <form action="{data.link_action}" method="post" name="form_search" id="form_search">
        <div class="row_search">
          <div class="col_search"><label>Danh sách dùng:</label> {data.list_use_search}</div>
        ...
        <table class="table table-bordered table-hover table-striped table_row">
          <thead>
            <tr >
              ...
              <th class="header" width="5%">{data.f_nature_id}</th>
          ...
          <tbody>
          	<!-- BEGIN: row_item -->
            <tr id="row_{row.nature_id}">
              <td class="cot" align="center"><input class="checkbox" type="checkbox" value="{row.nature_id}" name="selected_id[]"></td>
              <td class="cot" align="center">{row.nature_id}</td>
              <td class="cot" align="center"><input type="text" value="{row.show_order}" name="show_order[{row.nature_id}]" size="4" style="text-align:center;" onchange="do_check ({row.nature_id})"></td>
              <td class="cot">
                {row.title}
              </td>
              <td class="cot">
              	<div><strong>{LANG.global.date_create}:</strong> {row.date_create}</div>
              	<div><label for="is_focus_{row.nature_id}"><strong>{LANG.global.focus}:</strong></label> <input type="checkbox" value="1" id="is_focus_{row.nature_id}" name="is_focus[{row.nature_id}]" {row.is_focus_checked} onchange="do_check ('{row.nature_id}')" /></div>
              </td>
