...
<form action="{data.link_action}" method="post" enctype="multipart/form-data" name="myForm" id="myForm" role="form">
  <div class="row">
    ...
    <div class="col-md-6">
      <div class="form-group">
        <label>{row.title}</label>
        <input name="{row.i}" id="{row.i}" type="text" size="50" maxlength="150" value="{row.content}" class="form-control">
      </div>
      <p>Ví dụ: imsvietnamese.com, www.imsvietnamese.com, http://imsvietnamese</p>
    </div>
    <!-- END: link --> 
  </div>
  <!-- BEGIN: editor -->
  ...
  <div class="row">
    <div class="col-lg-12">
      <div class="bs-example tab_add">
        <ul class="nav nav-tabs" style="margin-bottom: 15px;">
          <!-- BEGIN: title -->
          <li class="{row.active}">
          	<a href="#editor_tab_{row.i}" data-toggle="tab">{row.title}</a>
          	<span class="fix_input"><input name="{row.key}[{row.i}][title]" type="text" size="20" maxlength="150" value="{row.title}" class="form-control input-sm"></span>
          </li>
          <!-- END: title --> 
          <li class="add_tab"><span>+</span></li>
        </ul>
        <div class="tab-content">
        	<!-- BEGIN: content -->
          <div class="tab-pane fade {row.active}" id="editor_tab_{row.i}">
            <textarea name="{row.key}[{row.i}][content]" id="{row.key}_{row.i}" class="form-control" rows="3">{row.content}</textarea>
            <script language="javascript">load_editor ("full", "{row.key}_{row.i}", g_folder_up, g_fldr);</script>
          </div>
          <!-- END: content --> 
        </div>
      </div>
    ...
    <script language="javascript">
    	$(document).on('click','.nav-tabs .add_tab',function(){
				var next = $(this).siblings('li').length+1;
				$(this).before('<li><a href="#editor_tab_'+next+'" data-toggle="tab">Tab trống</a><span class="fix_input"><input name="{row.key}['+next+'][title]" type="text" size="20" maxlength="150" value="" class="form-control input-sm"></span></li>');
				$(this).parent().siblings('.tab-content').append('<div class="tab-pane fade" id="editor_tab_'+next+'"><textarea name="{row.key}['+next+'][content]" id="{row.key}_'+next+'" class="form-control" rows="3"></textarea></div>');
				load_editor ("full", "{row.key}_"+next, g_folder_up, g_fldr);
			})
    	$(document).on('change','.nav-tabs li input',function(){
				$(this).parent().siblings('a').text($(this).val());
			})
    </script>
  </div><br />
  <!-- END: editor_tab --> 
