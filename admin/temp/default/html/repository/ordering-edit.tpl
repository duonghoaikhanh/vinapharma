  ...
  <table class="table table-bordered table-hover table-striped table_row">
    <thead>
      <tr >
        ...
        <th class="header" width="20%">{LANG.repository.col_price}</th>
        <th class="header" width="10%">{LANG.global.quantity}</th>
        <th class="header" width="20%">{LANG.repository.col_total}</th>
      </tr>
    </thead>
    <tbody>
      <!-- BEGIN: row_item -->
      <tr>
        ...
        <td class="cot price" align="center">{row.price_buy}</td>
      ...
      <tr>
        ...
        <td class="cot" align="center">
        	<input type="text" size="50" maxlength="20" value="{row.price_buy}" class="form-control price">
          <input name="detail[{row.detail_id}][price_buy]" type="hidden" value="{row.price_buy}" class="price_input">
        </td>
        <td class="cot" align="center">
        	<input type="text" size="50" maxlength="20" value="{row.quantity}" class="form-control auto_number">
          <input name="detail[{row.detail_id}][quantity]" type="hidden" value="{row.quantity}" class="auto_number_input">
        </td>
        <td class="cot" align="right">{row.total}</td>
      </tr>
      <!-- END: row_item_edit --> 
      <!-- BEGIN: row_empty -->
      <tr class="warning">
      ...
      <!-- END: row_empty --> 
      <tr>
        <td class="cot" align="right" colspan="4">       {LANG.repository.cart_total}</td>
        <td class="cot" align="right">{data.cart_total}</td>
      </tr>
    </tbody>
  </table>
</div>
<p class="text-danger">Cần lưu ý khi cập nhật lại giỏ hàng:</p>
<p class="text-danger">1. Chỉ có thể cập nhật lại đơn hàng khi đơn hàng ở trạng thái đơn hàng mới</p>
<p class="text-danger">2. Tổng giá sẽ bị thay đổi vì vậy nếu khách hàng đã thanh toán tiền thì cần lưu ý cách xoa dịu khách hàng</p>
<!-- END: table_cart --> 
