  ...
  <div class="row">
    ...
    <div class="col-md-6">
      <div class="form-group">
        ...
        <input name="name_action" id="name_action" type="text" size="50" maxlength="150" value="{data.name_action}" class="form-control">
      </div>
    </div>
    <div class="col-md-6">
      <div class="form-group">
    ...
    <div class="box_search">
      <form action="{data.link_action_search}" method="post" name="form_search" id="form_search">
        <div class="row_search">
          ...
          <div class="col_search"><label>{LANG.global.parent}:</label> {data.list_parent_search}</div>
        ...
        <table class="table table-bordered table-hover table-striped table_row">
          <thead>
            <tr >
              ...
              <th class="header" width="12%">{data.f_show_order}</th>
              <th class="header" width="15%">{data.f_name_action}</th>
              <th class="header" >{data.f_title}</th>
              <th class="header" width="30%">{data.f_link}</th>
              <th class="header" width="10%">{data.f_auto_sub}</th>
          ...
          <tbody>
          	{data.row_item}
          	<!-- BEGIN: row_item -->
            <tr id="row_{row.menu_id}">
              <td class="cot" align="center"><input class="checkbox" type="checkbox" value="{row.menu_id}" name="selected_id[]"></td>
              <td class="cot">{row.pre_title}<input type="text" value="{row.show_order}" name="show_order[{row.menu_id}]" size="4" style="text-align:center;" onchange="do_check ({row.menu_id})"></td>
              <td class="cot" align="center">{row.name_action}</td>
              <td class="cot">
              	<div>{row.pre_title}{row.title}</div>
                <div><label><input type="checkbox" value="1" name="lock_title[{row.menu_id}]" {row.lock_title_checked} onchange="do_check ('{row.menu_id}')" /> Khóa</label></div>
              </td>
              <td class="cot">{row.link}</td>
              <td class="cot">{row.list_auto_sub}</td>
