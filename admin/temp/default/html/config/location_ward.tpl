  ...
  <div class="row">
    ...
    <div class="col-lg-6">
      <div class="form-group">
        <label>{LANG.config.location_area}</label>
        {data.list_location_area}
      </div>
    </div>
    <div class="col-lg-6">
      <div class="form-group">
        <label>{LANG.config.location_country}</label>
        {data.list_location_country}
      </div>
    </div>
    <div class="col-lg-6">
      <div class="form-group">
        <label>{LANG.config.location_province}</label>
        {data.list_location_province}
      </div>
    </div>
    <div class="col-lg-6">
      <div class="form-group">
        <label>{LANG.config.location_district}</label>
        {data.list_location_district}
      </div>
    </div>
    <div class="col-lg-6">
      <div class="form-group">
        <label>{LANG.config.district_code}</label>
        <input name="code" id="code" type="text" size="50" maxlength="150" value="{data.code}" class="form-control" {data.disabled_code}>
      </div>
    </div>
    <div class="col-lg-6">
      <div class="form-group">
    ...
    <div class="box_search">
      <form action="{data.link_action_search}" method="post" name="form_search" id="form_search">
        <div class="row_search">
          <div class="col_search"><label>{LANG.config.location_area}:</label> {data.list_location_area_search}</div>
          <div class="col_search"><label>{LANG.config.location_country}:</label> {data.list_location_country_search}</div>
          <div class="col_search"><label>{LANG.config.location_province}:</label> {data.list_location_province_search}</div>
          <div class="col_search"><label>{LANG.config.location_district}:</label> {data.list_location_district_search}</div>
        ...
        <table class="table table-bordered table-hover table-striped table_row">
          <thead>
            <tr >
              ...
              <th class="header" width="15%">{data.f_code}</th>
              <th class="header" >{data.f_title}</th>
              <th class="header" width="15%">Nội / Ngoại thành</th>
          ...
          <tbody>
          	<!-- BEGIN: row_item -->
            <tr id="row_{row.code}">
              <td class="cot" align="center"><input class="checkbox" type="checkbox" value="{row.code}" name="selected_id[]"></td>
              <td class="cot" align="center"><input type="text" value="{row.show_order}" name="show_order[{row.code}]" size="4" style="text-align:center;" onchange="do_check ({row.code})"></td>
              <td class="cot" align="center">{row.code}</td>
              <td class="cot">{row.title}</td>
              <td class="cot" align="center">{row.is_extramural}</td>
