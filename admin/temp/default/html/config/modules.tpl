  ...
  <div class="row">
    ...
    <div class="col-lg-6">
      <div class="form-group">
        ...
        <p class="help-block">{LANG.config.name_action_note}</p>
      </div>
    </div>
  </div>
  <div class="row">
        ...
        <table class="table table-bordered table-hover table-striped table_row">
          <thead>
            <tr >
              ...
              <th class="header" width="10%">{data.f_name_action}</th>
              <th class="header" >{data.f_title}</th>
              <th class="header" width="20%">{data.f_friendly_link}</th>
              <th class="header" width="10%">{LANG.global.action}</th>
            </tr>
          </thead>
          <tbody>
          	<!-- BEGIN: row_item -->
            <tr id="row_{row.mod_id}">
              <td class="cot" align="center"><input class="checkbox" type="checkbox" value="{row.mod_id}" name="selected_id[]"></td>
              <td class="cot" align="center"><input type="text" value="{row.show_order}" name="show_order[{row.mod_id}]" size="4" style="text-align:center;" onchange="do_check ({row.mod_id})"></td>
              <td class="cot" align="center">{row.name_action}</td>
              <td class="cot">{row.title}</td>
              <td class="cot">{row.friendly_link}</td>
