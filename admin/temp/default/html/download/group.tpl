    ...
    <div class="col-lg-12">
      <div class="bs-example">
        <ul class="nav nav-tabs" style="margin-bottom: 15px;">
          ...
          <li><a href="#tab_content" data-toggle="tab">{LANG.global.content}</a></li>
        </ul>
    ...
    <div class="box_search">
      <form action="{data.link_action_search}" method="post" name="form_search" id="form_search">
        <div class="row_search">
          ...
          <div class="col_search"><label>{LANG.download.download_group}:</label> {data.list_group_search}</div>
        ...
        <table class="table table-bordered table-hover table-striped table_row">
          <thead>
            <tr >
              ...
              <th class="header" width="5%">{data.f_is_focus}</th>
              <th class="header" width="5%">{data.f_is_show_menu}</th>
          ...
          <tbody>
            ...
            <tr id="row_{row.group_id}">
              ...
              <td class="cot">{row.pre_title}<input type="text" value="{row.show_order}" name="show_order[{row.group_id}]" size="4" style="text-align:center;" onchange="do_check ({row.group_id})" /></td>
              ...
              <td class="cot">
              	<div><strong>{LANG.global.parent}:</strong> {row.parent}</div>
              	<div><strong>{LANG.global.date_create}:</strong> {row.date_create}</div>
              </td>
              <td class="cot" align="center">
              	<input class="checkbox" type="checkbox" value="1" name="is_focus[{row.group_id}]" {row.is_focus_checked} onchange="do_check ('{row.group_id}')" />
              </td>
              <td class="cot" align="center">
              	<input class="checkbox" type="checkbox" value="1" name="is_show_menu[{row.group_id}]" {row.is_show_menu_checked} onchange="do_check ('{row.group_id}')" />
              </td>
              <td class="cot" align="center">
