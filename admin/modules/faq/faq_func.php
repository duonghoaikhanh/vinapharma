...
function create_folder($dir="", $mode = 0755) {
	...
	if($ttH->func->rmkdir("faq/".$dir, $mode)){
		return $dir;
	}else{
		return "";
	}
...
function built_group_nav_sub ($group_id, $group_nav = '') {
	...
	while($row = $ttH->db->fetch_row($result)){
		...
		$ok = $ttH->db->do_update("faq_group", $col, " group_id='".$row['group_id']."'");
		if($ok) {
...
function get_data_group () {
	...
	if(!isset($ttH->data["faq_group"])){
		$query = "select m.group_id, group_nav, parent_id, title
							from faq_group m, faq_group_lang ml
...
function list_group ($select_name="group_id", $cur="", $ext="",$arr_more=array()) {
	...
	return $ttH->html->select ($select_name, $ttH->data["faq_group_tree"], $cur, $ext,$arr_more);
}
...
function get_group_name ($group_id, $link='') {
	...
	if (isset($ttH->data["faq_group"][$group_id])) {
		$row = $ttH->data["faq_group"][$group_id];
		if(!empty($link)) {
