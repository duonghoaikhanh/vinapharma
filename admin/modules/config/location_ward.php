		...
		if (isset($ttH->input['do_action']))
		{
		  ...
		  switch ($ttH->input["do_action"]){
				case "do_edit":
					...
					$arr_is_extramural = (isset($ttH->post["is_extramural"])) ? $ttH->post["is_extramural"] : array();
					...
					for ($i = 0; $i < count($up_id); $i ++){
						...
						$dup['is_extramural'] = $arr_is_extramural[$up_id[$i]];
    ...
    if ($num = $ttH->db->num_rows($result))
		{
			while ($row = $ttH->db->fetch_row($result)) 
			{
				...
				$row['is_extramural'] = $ttH->html->select('is_extramural['.$row['code'].']', $this->arr_extramural, $row['is_extramural'], ' class="forn-control" onchange="do_check (\''.$row['code'].'\')"');
