...
class sMain
{
	...
	**/
	...
	function do_add()
	{
		...
		if (isset($ttH->post['do_submit'])) {
			...
			$sql = "select code,title from location_area where (code='".$code."' or title='".$title."')";
			...
			if(empty($err)){
				...
				foreach($ttH->data["lang"] as $lang_id => $lang_row){
					...
					$ok = $ttH->db->do_insert("location_area", $col);	
				}
				if($ok){
					$data = array();
	...
	function do_edit()
	{
		...
		if (isset($ttH->post['do_submit'])) {
			...
			$sql = "select code,title from location_area where title='".$title."' and code!='".$code."'";
			$result = $ttH->db->query($sql);
			if ($check = $ttH->db->fetch_row($result)){
				if(empty($err) && $check['title'] == $title) {
					$err = $ttH->html->html_alert ('Tiêu đề đã tồn tại', "error");	
			...
			if(empty($err)){
				$col = array();
				$col["date_update"] = time();
				$ok = $ttH->db->do_update("location_area", $col, " code='".$code."'");	
				if($ok){
					$col_l = array();
					$col_l["title"] = $ttH->post["title"];
					
					$ttH->db->do_update("location_area", $col_l, " code='".$code."' and lang='".$ttH->conf["lang_cur"]."'");	
  ...
  function do_del ($list_del = "")
  {
    global $ttH;
		...
		$ok = $ttH->db->delete ("location_area", "find_in_set(code,'".$list_del."')");
    if ($ok){
	...
	function do_manage($is_show="")
	{
		  ...
		  switch ($ttH->input["do_action"]){
				case "do_edit":
					...
					for ($i = 0; $i < count($up_id); $i ++){
						...
						$ok = $ttH->db->do_update("location_area", $dup, "code='".$up_id[$i]."'");
						if ($ok){
							$str_mess .= ($str_mess) ? ", " : "";
							$str_mess .= $up_id[$i];
						} else{
		...
		$arr_title = array(
			"code" => array(
				"title" => $ttH->lang["config"]["area_code"],
