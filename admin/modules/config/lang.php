...
class sMain
{
	...
	**/
	...
	function do_setting()
	{
		...
		if(substr($file_name,0,7) == 'widget-') {
			$widget_name = substr($file_name,7);
			$file_lang = $ttH->conf["rootpath_web"] . DS . "lang". DS . $ttH->conf["lang_cur"] . DS . "widget". DS . $widget_name . ".php";
		} else {
			$file_lang = $ttH->conf["rootpath_web"] . DS . "lang". DS . $ttH->conf["lang_cur"] . DS . $file_name . ".php";
		}
