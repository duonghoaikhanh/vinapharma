...
class sMain
{
	...
	**/
	...
	function do_add()
	{
		...
		if (isset($ttH->post['do_submit'])) {
			...
			$sql = "select code,title from location_district where (code='".$code."' or title='".$title."')";
			...
			if(empty($err)){
				...
				foreach($ttH->data["lang"] as $lang_id => $lang_row){
					...
					$ok = $ttH->db->do_insert("location_district", $col);	
				}
				if($ok){
					$data = array();
		...
		$arr_isset = array('area_code', 'country_code', 'province_code');
		...
		$data["list_location_area"] = list_location_area ("area_code", $data['area_code']," class=\"form-control select_location_area\" data-country='country' data-province='province'");
		$data["list_location_country"] = list_location_country ("country_code", $data['area_code'], $data['country_code']," class=\"form-control select_location_country\" data-province='province' id='country'");
		$data["list_location_province"] = list_location_province ("province_code", $data['country_code'], $data['province_code']," class=\"form-control\" id='province'");
	...
	function do_edit()
	{
		...
		if (isset($ttH->post['do_submit'])) {
			...
			$sql = "select code,title from location_district where title='".$title."' and code!='".$code."'";
			$result = $ttH->db->query($sql);
			if ($check = $ttH->db->fetch_row($result)){
				if(empty($err) && $check['title'] == $title) {
					$err = $ttH->html->html_alert ('Tiêu đề đã tồn tại', "error");	
			...
			if(empty($err)){
				...
				$col["province_code"] = $province_code;
				$col["date_update"] = time();
				$ok = $ttH->db->do_update("location_district", $col, " code='".$code."'");	
				if($ok){
					$col_l = array();
					$col_l["title"] = $ttH->post["title"];
					
					$ttH->db->do_update("location_district", $col_l, " code='".$code."' and lang='".$ttH->conf["lang_cur"]."'");	
		...
		$sql = "select * from location_district 
  ...
  function do_del ($list_del = "")
  {
    global $ttH;
		...
		$ok = $ttH->db->delete ("location_district", "find_in_set(code,'".$list_del."')");
    if ($ok){
		  ...
		  switch ($ttH->input["do_action"]){
				case "do_edit":
					...
					for ($i = 0; $i < count($up_id); $i ++){
						...
						$ok = $ttH->db->do_update("location_district", $dup, "code='".$up_id[$i]."'");
						if ($ok){
							$str_mess .= ($str_mess) ? ", " : "";
							$str_mess .= $up_id[$i];
						} else{
    ...
    if ($num = $ttH->db->num_rows($result))
		...
		$data["list_location_area_search"] = list_location_area ("search_area_code", $search_area_code," class=\"form-control select_location_area\" data-country='search_country_code' data-province='search_province_code'");
		$data["list_location_country_search"] = list_location_country ("search_country_code", $search_area_code, $search_country_code," class=\"form-control select_location_country\" data-province='search_province_code' id='search_country_code'");
		$data["list_location_province_search"] = list_location_province ("search_province_code", $search_country_code, $search_province_code," class=\"form-control\" id='search_province_code'");
