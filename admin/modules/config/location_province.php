...
class sMain
{
	...
	**/
	...
	function do_add()
	{
		...
		if (isset($ttH->post['do_submit'])) {
			...
			$sql = "select code,title from location_province where (code='".$code."' or title='".$title."')";
			...
			if(empty($err)){
				...
				foreach($ttH->data["lang"] as $lang_id => $lang_row){
					...
					$ok = $ttH->db->do_insert("location_province", $col);	
				}
				if($ok){
					$data = array();
		...
		$arr_isset = array('area_code', 'country_code');
		...
		$data["list_location_area"] = list_location_area ("area_code", $data['area_code']," class=\"form-control select_location_area\" data-country='country'");
		$data["list_location_country"] = list_location_country ("country_code", $data['area_code'], $data['country_code']," class=\"form-control\" id='country'");
	...
	function do_edit()
	{
		...
		if (isset($ttH->post['do_submit'])) {
			...
			$sql = "select code,title from location_province where title='".$title."' and code!='".$code."'";
			$result = $ttH->db->query($sql);
			if ($check = $ttH->db->fetch_row($result)){
				if(empty($err) && $check['title'] == $title) {
					$err = $ttH->html->html_alert ('Tiêu đề đã tồn tại', "error");	
			...
			if(empty($err)){
				...
				$col["country_code"] = $country_code;
				$col["date_update"] = time();
				$ok = $ttH->db->do_update("location_province", $col, " code='".$code."'");	
				if($ok){
					$col_l = array();
					$col_l["title"] = $ttH->post["title"];
					
					$ttH->db->do_update("location_province", $col_l, " code='".$code."' and lang='".$ttH->conf["lang_cur"]."'");	
		...
		$sql = "select * from location_province 
  ...
  function do_del ($list_del = "")
  {
    global $ttH;
		...
		$ok = $ttH->db->delete ("location_province", "find_in_set(code,'".$list_del."')");
    if ($ok){
	...
	function do_manage($is_show="")
	{
		  ...
		  switch ($ttH->input["do_action"]){
				case "do_edit":
					...
					for ($i = 0; $i < count($up_id); $i ++){
						...
						$ok = $ttH->db->do_update("location_province", $dup, "code='".$up_id[$i]."'");
						if ($ok){
							$str_mess .= ($str_mess) ? ", " : "";
							$str_mess .= $up_id[$i];
						} else{
		...
		$arr_title = array(
			"code" => array(
				"title" => $ttH->lang["config"]["country_code"],
    ...
    if ($num = $ttH->db->num_rows($result))
		...
		$data["list_location_area_search"] = list_location_area ("search_area_code", $search_area_code," class=\"form-control select_location_area\" data-country='search_country_code'");
		$data["list_location_country_search"] = list_location_country ("search_country_code", $search_area_code, $search_area_code," class=\"form-control\" id='search_country_code'");
