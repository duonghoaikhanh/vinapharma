...
class sMain
{
	...
	**/
	...
	function table_cart ($order_id)
	{
		global $ttH;	
		
		$arr_color = array();
		$sql = "select color_id,color,arr_title 
						from product_color 
						where is_show=1 
		...
		$html_row = "";
		while ($row = $ttH->db->fetch_row($result)) {
			$arr_code_pic[$row['code_pic_id']] = $row;
		}
  ...
  function do_del ($list_del = "")
  {
	...
	function do_manage($is_show="")
	{
		...
		$arr_title = array(
			...
			"is_status" => array(
				"title" => $ttH->lang["product"]["status_order"],
			...
			"total_order" => array(
				"title" => $ttH->lang["product"]["total_order"],
