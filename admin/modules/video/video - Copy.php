...
class sMain
{
	...
	**/
	...
	function do_add()
	{
		...
		if (isset($ttH->post['do_submit'])) {
			...
			$video = (isset($ttH->post['video'])) ? $ttH->post['video'] : '';
			...
			if(empty($video)) {
				$err = $ttH->html->html_alert ('Vui lòng nhập video', "error");	
			}	
			
			$picture = $ttH->func->get_input_pic ($ttH->post["picture"]);
			if(empty($ttH->post["picture"])) {
				$picture = (isset($ttH->post['video'])) ? $ttH->func->get_youtube_pic($ttH->post['video'], 'video') : '';
			}	
			...
			if(empty($err)){
				...
				if($ok){
					...
					foreach($ttH->data["lang"] as $lang_id => $lang_row){
						...
						$col_l["friendly_link"] = $ttH->func->get_friendly_link_db ($friendly_link, 'video_lang', 'item_id', $item_id, $lang_row["name"]);
						
						$col_l["lang"] = $lang_row["name"];
						$ttH->db->do_insert("video_lang", $col_l);	
					}
	...
	function do_edit()
	{
		...
		if (isset($ttH->post['do_submit'])) {
			...
			if(empty($err)){
				...
				$col["video"] = $video;
				$col["date_update"] = time();
				$ok = $ttH->db->do_update("video", $col, " item_id='".$item_id."'");	
				if($ok){
					$col_l = array();
					$col_l["title"] = $ttH->post["title"];
					$col_l["content"] = $ttH->func->input_editor ($ttH->post["content"]);
					...
					$friendly_link = ($ttH->post["friendly_link"]) ? $ttH->post["friendly_link"] : $ttH->post["title"];
					$col_l["friendly_link"] = $ttH->func->get_friendly_link_db ($friendly_link, 'video_lang', 'item_id', $item_id, $ttH->conf['lang_cur']);
					
					$col_l["meta_title"] = ($ttH->post["meta_title"]) ? $ttH->post["meta_title"] : $ttH->func->meta_title ($ttH->post["title"]);
					$col_l["meta_key"] = ($ttH->post["meta_key"]) ? $ttH->post["meta_key"] : $ttH->func->meta_key ($ttH->post["title"]);
					$col_l["meta_desc"] = ($ttH->post["meta_desc"]) ? $ttH->func->meta_desc ($ttH->post["meta_desc"]) : $ttH->func->meta_desc ($ttH->post["content"]);
					
					$ttH->db->do_update("video_lang", $col_l, " item_id='".$item_id."' and lang='".$ttH->conf["lang_cur"]."'");	
		...
		$sql = "select * from video a, video_lang al 
  ...
  function do_del ($list_del = "")
  {
    global $ttH;
		...
		if(empty($list_del)){
			$ttH->html->alert ($ttH->lang["global"]["not_found_video"], $ttH->admin->get_link_admin ($this->modules, $this->action));
		}
		$del_item = "";
		$del_item_lang = "";
		
		$query = $ttH->db->query("select id, item_id from video_lang where find_in_set(item_id,'".$list_del."') and lang='".$ttH->conf["lang_cur"]."'");	
		while ($row = $ttH->db->fetch_row($query)){
			$check = $ttH->db->query("select id from video_lang where item_id='".$row["item_id"]."' and lang!='".$ttH->conf["lang_cur"]."'");
		...
		$ok = $ttH->db->delete ("video_lang", "find_in_set(id,'".$del_item_lang."')");
    if ($ok){
			$ttH->db->delete ("video", "find_in_set(item_id,'".$del_item."')");
