...
function load_setting (){
	...
	$ttH->data["menu_group"] = array(
		...
		//'home_list_left' => 'Trang chủ (bên trái hình)',
		//'home_list_right' => 'Trang chủ (bên phải hình)'
	);
...
function list_auto_sub ($select_name="group_id", $cur="", $ext="",$arr_more=array()) {
	global $ttH;
...
function list_menu ($group_id, $select_name="menu_id", $cur="", $ext="",$arr_more=array()) {
	...
	if(!isset($ttH->data["menu_tree"])){
		$query = "select menu_id, menu_nav, parent_id, title 
							from menu 
							where is_show=1 
