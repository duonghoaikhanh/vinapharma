...
function load_setting (){
	...
	if(!isset($ttH->data["dealer_group"])){
		$result = $ttH->db->query("select p.group_id, title 
																from dealer_group p, dealer_group_lang pl  
																where p.group_id=pl.group_id 
																and is_show=1 
																order by show_order desc, date_create asc");
		$ttH->data["dealer_group"] = array();
		while ($row = $ttH->db->fetch_row($result))
		{
			$ttH->data["dealer_group"][$row['group_id']] = $row['title'];
		}
	}
...
function create_folder($dir="", $mode = 0755) {
	...
	if($ttH->func->rmkdir("dealer/".$dir, $mode)){
		return $dir;
	}else{
		return "";
	}
...
function built_group_nav_sub ($group_id, $group_nav = '') {
	...
	$ttH->db->do_update("dealer", $col_up, " group_id='".$group_id."'");	
	//End 
	...
	while($row = $ttH->db->fetch_row($result)){
		...
		$ok = $ttH->db->do_update("dealer_group", $col, " group_id='".$row['group_id']."'");	
		if($ok) {
			
		//update dealer
			$col_up = array();
			$col_up["group_nav"] = $col["group_nav"];
			$ttH->db->do_update("dealer", $col_up, " group_id='".$row['group_id']."'");	
			//End 
...
function get_group_name ($group_id, $type='none'){
	...
	$data = $ttH->load_data->data_table ('dealer_group_lang', 'group_id', 'group_id,title,friendly_link', " lang='".$ttH->conf['lang_cur']."' and group_id='".$group_id."' ");
	...
	if (isset($data[$group_id])) {
		...
		switch ($type) {
			case "link":
				$link = $ttH->admin->get_link_admin ('dealer', 'group', 'edit', array("id"=>$row['group_id']));
...
function list_group ($select_name="group_id", $cur="", $ext="",$arr_more=array()) {
	...
	if(!isset($ttH->data["dealer_group_tree"])){
		$query = "select m.group_id, group_nav, parent_id, title 
							from dealer_group m, dealer_group_lang ml  
		...
		$result = $ttH->db->query($query);
		$ttH->data["dealer_group"] = array();
		$ttH->data["dealer_group_tree"] = array();
		if($num = $ttH->db->num_rows($result)){
			while($row = $ttH->db->fetch_row($result)){
				$ttH->data["dealer_group"][$row["group_id"]] = $row;
				...
				eval('$ttH->data["dealer_group_tree"]'.$str_code.'["group_id"] = $row["group_id"];
				$ttH->data["dealer_group_tree"]'.$str_code.'["title"] = $row["title"];');
			}
		}
	}
	$select_muti = (isset($arr_more['select_muti'])) ? $arr_more['select_muti'] : 0;
	if($select_muti == 1) {
		return $ttH->html->select_muti ($select_name, $ttH->data["dealer_group_tree"], $cur, $ext,$arr_more);
	} else {
		return $ttH->html->select ($select_name, $ttH->data["dealer_group_tree"], $cur, $ext,$arr_more);
	}
}
