			...
			while ($row = $ttH->db->fetch_row($result)) 
			{
				...
				foreach($this->arr_element as $k => $v) {
					...
					if($form_type == 'checkbox') {
						...
						$row['html_info'] .= '<div><label for="'.$k.'_'.$row[$this->dbtable_id].'"><strong>'.(isset($ttH->lang[$this->modules][$k]) ? $ttH->lang[$this->modules][$k] : (isset($ttH->lang['global'][$k]) ? $ttH->lang['global'][$k] : $k)).':</strong></label> <input type="checkbox" value="1" id="'.$k.'_'.$row[$this->dbtable_id].'" name="'.$k.'['.$row[$this->dbtable_id].']" '.$row[$k.'_checked'].' onchange="do_check(\''.$row[$this->dbtable_id].'\')" /></div>';
					}elseif($form_type == 'color') {
						$row['html_info'] .= '<div><label><strong>'.(isset($ttH->lang[$this->modules][$k]) ? $ttH->lang[$this->modules][$k] : (isset($ttH->lang['global'][$k]) ? $ttH->lang['global'][$k] : $k)).':</strong></label> <span style="border:1px solid #000; background:'.$row['color'].'">&nbsp; &nbsp; &nbsp;</span></div>';
					} else {
						continue;
					}
				}
