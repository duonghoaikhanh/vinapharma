...
class sMain
{
		...
		if (isset($ttH->post['do_submit'])) {
			...
			if(empty($err)){
				...
				foreach($ttH->data["lang"] as $lang_id => $lang_row){
					...
					$ok = $ttH->db->do_insert("repository_method_group", $col);	
					if($ok && $col["group_id"] == 0){
						...
						$ttH->db->do_update("repository_method_group", $col_l, " id='".$group_id."'");	// update current
					}
				}
				if($ok){
					$data = array();
	...
	function do_edit()
	{
		...
		if (isset($ttH->post['do_submit'])) {
			...
			if(empty($err)){
				...
				$col["picture"] = $ttH->func->get_input_pic ($ttH->post["picture"]);
				$col["date_update"] = time();
				$ok = $ttH->db->do_update("repository_method_group", $col, " group_id='".$group_id."'");	
				if($ok){
					$col_l = array();
					...
					$ttH->db->do_update("repository_method_group", $col_l, " group_id='".$group_id."' and lang='".$ttH->conf["lang_cur"]."'");	
		...
		$sql = "select * from repository_method_group 
  ...
  function do_del ($list_del = "")
  {
    global $ttH;
		...
		$ok = $ttH->db->delete ("repository_method_group", "find_in_set(group_id,'".$list_del."')");
    if ($ok){
	...
	function do_manage($is_show="")
	{
		  ...
		  switch ($ttH->input["do_action"]){
				case "do_edit":
					...
					for ($i = 0; $i < count($up_id); $i ++){
						...
						$ok = $ttH->db->do_update("repository_method_group", $dup, "group_id=" . $up_id[$i]);
						if ($ok){
							$str_mess .= ($str_mess) ? ", " : "";
							$str_mess .= $up_id[$i];
						} else{
		...
		if(!empty($search_title)){
			$where .=" and (group_id='$search_title' or title like '%$search_title%') ";			
		...
		if($sort)
		{
			...
			if (array_key_exists($tmp[0],$arr_title) && in_array($tmp[1],$arr_allow_sort)) {
				$order_tmp = ($tmp[0] == "group_id") ? "group_id" : $tmp[0];
    ...
    if ($num = $ttH->db->num_rows($result))
		{
			while ($row = $ttH->db->fetch_row($result)) 
			{
				...
				$row["link_pic"] = $ttH->admin->get_link_admin ($this->modules, $this->action.'_pic', 'manage', array("type"=>"item","type_id"=>$row['group_id']));
