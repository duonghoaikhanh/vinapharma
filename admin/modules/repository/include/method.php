...
class sMain_sub extends sMain {
	...
	//var $action = "method";
	//var $sub = "manage";
	var $folder_upload = 'repository/method';
	var $dir = "";
	...
	function do_add()
	{
		...
		if (isset($ttH->post['do_submit'])) {
			...
			if(empty($err)){
				...
				$col = array();
				$col["method_id"] = 0;
				$col["group_id"] = $ttH->func->input_text ($ttH->post["group_id"]);
				$col["method_type"] = $this->method_type;
				...
				foreach($ttH->data["lang"] as $lang_id => $lang_row){
					...
					$ok = $ttH->db->do_insert("repository_method", $col);	
					if($ok && $col["method_id"] == 0){
						...
						$ttH->db->do_update("repository_method", $col_l, " id='".$method_id."'");	// update current
					}
				}
				if($ok){
					$data = array();
		...
		$data["list_group"] = list_group_method ("group_id", $data["group_id"], " class=\"form-control\"", array("title" => $ttH->lang['global']['select_title']));
	...
	function do_edit()
	{
		...
		if (isset($ttH->post['do_submit'])) {
			...
			if(empty($err)){
				...
				$col["picture"] = $ttH->func->get_input_pic ($ttH->post["picture"]);
				$col["date_update"] = time();
				$ok = $ttH->db->do_update("repository_method", $col, " method_id='".$method_id."'");	
				if($ok){
					$col_l = array();
					...
					$ttH->db->do_update("repository_method", $col_l, " method_id='".$method_id."' and method_type='".$this->method_type."' and lang='".$ttH->conf["lang_cur"]."'");	
		...
		$sql = "select * from repository_method 
						where lang='".$ttH->conf['lang_cur']."' 
						and method_type='".$this->method_type."' 
  ...
  function do_del ($list_del = "")
  {
    global $ttH;
		...
		$ok = $ttH->db->delete ("repository_method", "method_type='".$this->method_type."' and find_in_set(method_id,'".$list_del."')");
    if ($ok){
	...
	function do_manage($is_show="")
	{
		  ...
		  switch ($ttH->input["do_action"]){
				case "do_edit":
					...
					for ($i = 0; $i < count($up_id); $i ++){
						...
						$ok = $ttH->db->do_update("repository_method", $dup, "method_id=" . $up_id[$i]);
						if ($ok){
							$str_mess .= ($str_mess) ? ", " : "";
							$str_mess .= $up_id[$i];
						} else{
		...
		$where = " where lang='".$ttH->conf["lang_cur"]."' and method_type='".$this->method_type."' ";
		$ext = "";
		$is_search = 0;
    ...
    if ($num = $ttH->db->num_rows($result))
		...
		$data["list_group_search"] = list_group_method ("search_group_id", $search_group_id, " class=\"form-control\"", array("title" => $ttH->lang['global']['select_title']));
