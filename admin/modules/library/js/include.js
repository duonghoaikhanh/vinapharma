...
$(document).ready(function(){
		...
		var options = {
		  callback: function(key, options) {
			...
			case "unzip":
			    ...
			    $.ajax({
				type: "POST",
				url: "?mod=library&act=library&sub=ajax_calls&folder_up="+folder_up+"&action=extract",
				data: { path: m }
			    }).done(function( msg ) {
				if (msg!="")
				    bootbox.alert(msg);
	...
	if (js_script) {
	    $.ajax({
		url: "?mod=library&act=library&sub=ajax_calls&action=sort&sort_by="+_this.attr('data-sort')+"&descending="+sortDescending
	    }).done(function( msg ) {
	...
	bootbox.prompt($('#insert_folder_name').val(),$('#cancel').val(),$('#ok').val(), function(name) {
		...
		$.ajax({
			  type: "POST",
			  url: "?mod=library&act=library&sub=execute&folder_up="+folder_up+"&action=create_folder",
    ...
    $('.view-controller button').on('click',function(){
	    ...
	    _this.find('i').addClass('icon-white');
	     ...
	     $.ajax({
		url: "?mod=library&act=library&sub=ajax_calls&action=view&type="+_this.attr('data-value')
	    }).done(function( msg ) {
		if (msg!="") {
		    bootbox.alert(msg);
		}   
...
function apply_muti(file,external){
    if ($('#popup').val()==1) var window_parent=window.opener; else var window_parent=window.parent;
    var path = $('#cur_dir').val();    
    //path = path.replace('\\', '/');
    var base_url = $('#base_url').val();
    ...
    parent.tinymce.activeEditor.insertContent(fill);
		$('#tth-mess').html('<div class="tth-mess mess-success">Đã thêm file <strong>'+file+'</strong> vào trình soạn thảo</div>');
    //parent.tinymce.activeEditor.windowManager.close();
}
...
function execute_action(action,file1,file2,name,container,function_name){
	...
	$.ajax({
	    type: "POST",
	    url: "?mod=library&act=library&sub=execute&folder_up="+folder_up+"&action="+action,
