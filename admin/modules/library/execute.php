...
$thumb_pos=strpos($ttH->post['path_thumb'],$thumbs_base_path);
if($thumb_pos !=0
    || strpos($ttH->post['path_thumb'],'../',strlen($thumbs_base_path)+$thumb_pos)!==FALSE
...
if (isset($ttH->get['lang']) && $ttH->get['lang'] != 'undefined' && $ttH->get['lang']!='') {
    $path_parts = pathinfo($ttH->get['lang']);
...
$path=$current_path.$ttH->post['path'];
$path_thumb=$ttH->post['path_thumb'];
$path_thumbs_size=$current_thumbs_size.$ttH->post['path'];
if(isset($ttH->post['name'])){
    $name=$ttH->post['name'];
    if(strpos($name,'../')!==FALSE) die('wrong name');
}

$info=pathinfo($path);
if(isset($info['extension']) && !(isset($ttH->get['action']) && $ttH->get['action']=='delete_folder') && !in_array(strtolower($info['extension']), $ext)){
    die('wrong extension');
}
...
if(isset($ttH->get['action'])){
    ...
    switch($ttH->get['action']){
        case 'delete_file':
						...
						$path_thumbs_size .= '_'.substr($ttH->post['path'], strrpos($ttH->post['path'], ".") + 1);
            if($delete_files){
                unlink($path);
