...
if (isset($ttH->get['fldr'])
		&& !empty($ttH->get['fldr'])
		&& strpos($ttH->get['fldr'],'../')===FALSE
		&& strpos($ttH->get['fldr'],'./')===FALSE)
		$subdir = urldecode(trim($ttH->get['fldr'],"/") ."/");
else
		$subdir = '';
...
if(isset($ttH->get['popup'])) $popup= $ttH->get['popup']; else $popup=0;

//view type
if(!isset($_SESSION["view_type"])){ $view=$default_view; $_SESSION["view_type"] = $view; }
if(isset($ttH->get['view'])){ $view=$ttH->get['view']; $_SESSION["view_type"] = $view; }
$view=$_SESSION["view_type"];

if(isset($ttH->get["filter"])) $filter=fix_filename($ttH->get["filter"],$transliteration);
else $filter='';

if(!isset($_SESSION['sort_by'])) $_SESSION['sort_by']='';
if(isset($ttH->get["sort_by"])) $sort_by=$_SESSION['sort_by']=fix_filename($ttH->get["sort_by"],$transliteration);
else $sort_by=$_SESSION['sort_by'];

if(!isset($_SESSION['descending'])) $_SESSION['descending']=false;
if(isset($ttH->get["descending"])) $descending=$_SESSION['descending']=fix_filename($ttH->get["descending"],$transliteration)==="true";
else $descending=$_SESSION['descending'];


$lang=$default_language;
if(isset($ttH->get['lang']) && $ttH->get['lang'] != 'undefined' && $ttH->get['lang']!='')
		$lang=$ttH->get['lang'];
...
if(!isset($ttH->get['type'])) $ttH->get['type']=0;
if(!isset($ttH->get['field_id'])) $ttH->get['field_id']='';

$get_params = http_build_query(array(
		'type'      => $ttH->get['type'],
		'lang'      => $lang,
		'popup'     => $popup,
		'field_id'  => isset($ttH->get['field_id']) ? $ttH->get['field_id'] : '',
		'folder_up'     => $ttH->conf["folder_up"],
		'fldr'      => ''
));
?>
...
<html xmlns="https://www.w3.org/1999/xhtml">
	...
	<link rel="shortcut icon" href="<?php echo $ttH->dir_mod?>img/ico/favicon.ico">
	<link href="<?php echo $ttH->dir_mod?>css/bootstrap.min.css" rel="stylesheet" type="text/css" />
	<link href="<?php echo $ttH->dir_mod?>css/bootstrap-responsive.min.css" rel="stylesheet" type="text/css" />
	<link href="<?php echo $ttH->dir_mod?>css/bootstrap-lightbox.min.css" rel="stylesheet" type="text/css" />
	<link href="<?php echo $ttH->dir_mod?>css/style.css" rel="stylesheet" type="text/css" />
	<link href="<?php echo $ttH->dir_mod?>css/dropzone.min.css" type="text/css" rel="stylesheet" />
	<link href="<?php echo $ttH->dir_mod?>css/jquery.contextMenu.min.css" rel="stylesheet" type="text/css" />	
	<link href="<?php echo $ttH->dir_mod?>css/bootstrap-modal.min.css" rel="stylesheet" type="text/css" />
	<link href="<?php echo $ttH->dir_mod?>jPlayer/skin/blue.monday/jplayer.blue.monday.css" rel="stylesheet" type="text/css">
	<!--[if lt IE 8]><style>
	...
	<script type="text/javascript" src="<?php echo $ttH->dir_mod?>js/jquery.js"></script>
	<script type="text/javascript" src="<?php echo $ttH->dir_mod?>js/bootstrap.min.js"></script>
	<script type="text/javascript" src="<?php echo $ttH->dir_mod?>js/bootstrap-lightbox.min.js"></script>
	<script type="text/javascript" src="<?php echo $ttH->dir_mod?>js/dropzone.min.js"></script>
	<script type="text/javascript" src="<?php echo $ttH->dir_mod?>js/jquery.touchSwipe.min.js"></script>
	<script type="text/javascript" src="<?php echo $ttH->dir_mod?>js/modernizr.custom.js"></script>
	<script type="text/javascript" src="<?php echo $ttH->dir_mod?>js/bootbox.min.js"></script>
	<script type="text/javascript" src="<?php echo $ttH->dir_mod?>js/bootstrap-modal.min.js"></script>   
	<script type="text/javascript" src="<?php echo $ttH->dir_mod?>js/bootstrap-modalmanager.min.js"></script>
	<script type="text/javascript" src="<?php echo $ttH->dir_mod?>jPlayer/jquery.jplayer.min.js"></script>
	<script type="text/javascript" src="<?php echo $ttH->dir_mod?>js/imagesloaded.pkgd.min.js"></script>
	<script type="text/javascript" src="<?php echo $ttH->dir_mod?>js/jquery.queryloader2.min.js"></script>
	
	<!-- Le HTML5 shim, for IE6-8 support of HTML5 elements -->
	<!--[if lt IE 9]>
		<script src="<?php echo $ttH->dir_mod?>js/html5shiv.js"></script>
	<![endif]-->
	<script src="<?php echo $ttH->dir_mod?>js/jquery.ui.position.min.js" type="text/javascript"></script>
	<script src="<?php echo $ttH->dir_mod?>js/jquery.contextMenu.min.js" type="text/javascript"></script>    
	
	<script>
			var folder_up = "<?php echo $ttH->conf["folder_up"]; ?>";
			...
			Dropzone.options.myAwesomeDropzone = {
				...
				url: "<?php echo $this->link_act?>&sub=upload&folder_up=<?php echo $ttH->conf["folder_up"]?>",
	...
	<script type="text/javascript" src="<?php echo $ttH->dir_mod?>js/include.js"></script>
		</head>
...
<div class="uploader">
	...
	<div class="tabbable upload-tabbable"> <!-- Only required for left/right tabs -->
				<?php } ?>
		<form action="<?php echo $this->link_act?>&sub=popup_library" method="post" enctype="multipart/form-data" id="myAwesomeDropzone" class="dropzone">
			...
			<input type="hidden" name="type" value="<?php echo $ttH->get['type']; ?>"/>
			<input type="hidden" name="field_id" value="<?php echo $ttH->get['field_id']; ?>"/>
...
if ($ttH->get['type']==1) 	 $apply = 'apply_img';
elseif($ttH->get['type']==2) $apply = 'apply_link';
elseif($ttH->get['type']==0 && $ttH->get['field_id']=='') $apply = 'apply_none';
elseif($ttH->get['type']==3) $apply = 'apply_video';
elseif($ttH->get['type']==4) $apply = 'apply_muti';
else $apply = 'apply';
...
<div class="navbar navbar-fixed-top">
		...
		<div class="filters">
			...
			<div class="span6 types">
					...
					<?php if($ttH->get['type']!=1 && $ttH->get['type']!=3){ ?>
					...
					<input id="select-type-all" name="radio-sort" type="radio" data-item="ff-item-type-all" class="hide"  />
					 <label id="ff-item-type-all" title="<?php echo lang_All; ?>" <?php if($ttH->get['type']==1 || $ttH->get['type']==3){ ?>style="visibility: hidden;" <?php } ?> data-item="ff-item-type-all" for="select-type-all" style="margin-rigth:0px;" class="tip btn btn-inverse ff-label-type-all"><i class="icon-align-justify icon-white"></i></label>
...
<!----- header div end ------->
	<div id="tth-mess" class="row-fluid"></div>
		<!----- breadcrumb div start ------->
		
		<div class="row-fluid">
	<?php	
	$link=$this->link_act."&sub=popup_library&".$get_params;
	?>
	<ul class="breadcrumb">	
	...
	<li class="pull-right"><a id="refresh" class="btn-small" href="<?php echo $this->link_act."&sub=popup_library&".$get_params.$subdir."&".uniqid() ?>"><i class="icon-refresh"></i></a></li>
		...
		foreach ($files as $file_array) {
			...
			<li data-name="<?php echo $file ?>" <?php if($file=='..') echo 'class="back"'; else echo 'class="dir"'; ?>>
				<figure data-name="<?php echo $file ?>" class="<?php if($file=="..") echo "back-"; ?>directory" data-type="<?php if($file!=".."){ echo "dir"; } ?>">
						<a class="folder-link" href="<?php echo $this->link_act."&sub=popup_library&".$get_params.rawurlencode($src)."&".uniqid() ?>">
						<div class="img-precontainer">
					<div class="img-container directory"><span></span>
					<img class="directory-img"  src="<?php echo $ttH->dir_mod?>img/<?php echo $icon_theme; ?>/folder<?php if($file==".."){ echo "_back"; }?>.jpg" alt="folder" />
					</div>
						</div>
						<div class="img-precontainer-mini directory">
					<div class="img-container-mini">
							<span></span>
							<img class="directory-img"  src="<?php echo $ttH->dir_mod?>img/<?php echo $icon_theme; ?>/folder<?php if($file==".."){ echo "_back"; }?>.png" alt="folder" />
					</div>
						</div>
			<?php if($file==".."){ ?>
			...
			<?php }else{ ?>
						...
						<div class="box">
					<h4 class="<?php if($ellipsis_title_after_first_row){ echo "ellipsis"; } ?>"><a class="folder-link" data-file="<?php echo $file ?>" href="<?php echo $this->link_act."&sub=popup_library&".$get_params.rawurlencode($src)."&".uniqid() ?>"><?php echo $file; ?></a></h4>
						</div>
			...
			$file=$file_array['file'];
				...
				$src_thumb=$thumbs_base_url.$subfolder.$subdir.$file;
				
				$is_img=true;
				//check if is smaller than thumb
				list($img_width, $img_height, $img_type, $attr)=getimagesize($file_path);
				if($img_width<122 && $img_height<91){ 
					$src_thumb=$current_src.$subfolder.$subdir.$file;
					$show_original=true;
				}
				
				if($img_width<45 && $img_height<38){
						$mini_src=$current_src.$subfolder.$subdir.$file;
					...
					if($src_thumb==""){
				if(file_exists($ttH->dir_mod.'img/'.$icon_theme.'/'.$extension_lower.".jpg")){
					$src_thumb =$ttH->dir_mod.'img/'.$icon_theme.'/'.$extension_lower.".jpg";
				}else{
					$src_thumb = $ttH->dir_mod."img/".$icon_theme."/default.jpg";
				}
				$is_icon_thumb=true;
					}
					if($mini_src==""){
				$is_icon_thumb_mini=false;
					...
					if((!($ttH->get['type']==1 && !$is_img) && !(($ttH->get['type']==3 && !$is_video) && ($ttH->get['type']==3 && !$is_audio))) && $class_ext>0){
?>
		<li class="ff-item-type-<?php echo $class_ext; ?> file"  data-name="<?php echo $file; ?>">
			<figure data-name="<?php echo $file ?>" data-type="<?php if($is_img){ echo "img"; }else{ echo "file"; } ?>">
				<a href="javascript:void('')" class="link" data-file="<?php echo $file; ?>" data-field_id="<?php echo $ttH->get['field_id']; ?>" data-function="<?php echo $apply; ?>">
				<div class="img-precontainer">
				...
				<div class="box">				
				<h4 class="<?php if($ellipsis_title_after_first_row){ echo "ellipsis"; } ?>"><a href="javascript:void('')" class="link" data-file="<?php echo $file; ?>" data-field_id="<?php echo $ttH->get['field_id']; ?>" data-function="<?php echo $apply; ?>">
				...
				<figcaption>
						<form action="<?php echo $ttH->dir_mod?>force_download.php" method="post" class="download-form" id="form<?php echo $nu; ?>">
					...
					<input type="hidden" class="name_download" name="name" value="<?php echo $file?>"/>
						...
						<a class="tip-right modalAV <?php if($is_audio){ echo "audio"; }else{ echo "video"; } ?>" title="<?php echo lang_Preview?>" data-url="<?php echo $this->link_act?>&ajax_calls&action=media_preview&title=<?php echo $filename; ?>&file=<?php echo $current_path.$subfolder.$subdir.$file;; ?>" href="javascript:void('');" ><i class=" icon-eye-open"></i></a>
						<?php }else{ ?>
	...
	?></div>
		...
		<div id="loading_container" style="display:none;">
			...
			<img id="loading_animation" src="<?php echo $ttH->dir_mod?>img/storing_animation.gif" alt="loading" style="z-index:10001; margin-left:-32px; margin-top:-32px; position:fixed; left:50%; top:50%"/>
		</div>
		<!----- loading div end ------->
