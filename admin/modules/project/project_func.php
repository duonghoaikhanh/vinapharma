...
function load_setting (){
	...
	if(!isset($ttH->data["project_group"])){
		$result = $ttH->db->query("select p.group_id, title 
																from project_group p, project_group_lang pl  
																where p.group_id=pl.group_id 
																and is_show=1 
																order by show_order desc, date_create asc");
		$ttH->data["project_group"] = array();
		while ($row = $ttH->db->fetch_row($result))
		{
			$ttH->data["project_group"][$row['group_id']] = $row['title'];
		}
	}
...
function create_folder($dir="", $mode = 0755) {
	...
	if($ttH->func->rmkdir("project/".$dir, $mode)){
		return $dir;
	}else{
		return "";
	}
...
function get_group_name ($group_id, $type='none'){
	...
	$data = $ttH->load_data->data_table ('project_group_lang', 'group_id', 'group_id,title,friendly_link', " lang='".$ttH->conf['lang_cur']."' and group_id='".$group_id."' ");
	...
	if (isset($data[$group_id])) {
		...
		switch ($type) {
			case "link":
				$link = $ttH->admin->get_link_admin ('project', 'group', 'edit', array("id"=>$row['group_id']));
...
function list_group ($select_name="group_id", $cur="", $ext="",$arr_more=array()) {
	...
	if(!isset($ttH->data["project_group_tree"])){
		$query = "select m.group_id, group_nav, parent_id, title 
							from project_group m, project_group_lang ml  
		...
		$result = $ttH->db->query($query);
		$ttH->data["project_group"] = array();
		$ttH->data["project_group_tree"] = array();
		if($num = $ttH->db->num_rows($result)){
			while($row = $ttH->db->fetch_row($result)){
				$ttH->data["project_group"][$row["group_id"]] = $row;
				...
				eval('$ttH->data["project_group_tree"]'.$str_code.'["group_id"] = $row["group_id"];
				$ttH->data["project_group_tree"]'.$str_code.'["title"] = $row["title"];');
			}
		}
	}
	$select_muti = (isset($arr_more['select_muti'])) ? $arr_more['select_muti'] : 0;
	if($select_muti == 1) {
		return $ttH->html->select_muti ($select_name, $ttH->data["project_group_tree"], $cur, $ext,$arr_more);
	} else {
		return $ttH->html->select ($select_name, $ttH->data["project_group_tree"], $cur, $ext,$arr_more);
	}
}
